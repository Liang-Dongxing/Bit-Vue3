<template>
  <div class="icon-body">
    <el-input v-model="iconName" style="position: relative" clearable :placeholder="$t('om.please_enter')" @clear="filterIcons" @input="filterIcons">
      <template #suffix><i class="el-icon-search el-input__icon" /></template>
    </el-input>
    <div class="icon-list" v-infinite-scroll="load" infinite-scroll-distance="50">
      <div v-for="(item, index) in iconList" :key="index" @click="selectedIcon(item)">
        <icon-park :type="item" :size="16" theme="filled" />
        <span>{{ item }}</span>
      </div>
    </div>
  </div>
