...
const data = reactive({
  ...
  rules: {
    ...
    accessKeyType: [
      {required: true, message: "AccessKey 平台不能为空", trigger: "change"}
    ],
  }
});
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除第三方AccessKey管理编号为"' + _accessIds + '"的数据项？').then(function () {
...
function handleExport() {
  proxy.download('ops/access/export', {
    ...queryParams.value
